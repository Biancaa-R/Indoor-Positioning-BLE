...
RTC_DATA_ATTR const char* pubTopic = "fromDEV1"; //wrt to the node red flow
...
void setup(){
  ...
  */
  ...
  BLEScanResults foundDevices = pBLEScan->start(5, false); //perform the actual scan, false is to clear already scanned devices
...
while(dev_count!=0){
    ...
    for(int i=0;i<MAX_NO_DEV;i++){   //publishing MQTT MSGS for devices found.
      ...
      snprintf(pubmsg, MQTT_MSG_BUF,"{\"UUID\":\"%s\",\"SCANNER\":\"1\",\"RSSI\":\"%d\",\"RANGE\":\"%s\"}",temp1,meanRSSI,detectedUUID[i][5]);
      ...
      while (millis() - loopStart < 10) {   //MQTTclient.loop must be called regularly to maintain connection
 ...
 /* uint32_t loopStart = millis(); 
  ...
  snprintf(lastwill, MQTT_MSG_BUF,"{\"UUID\":\"%s\",\"SCANNER\":\"1\",\"RSSI\":\"-120\"}",temp2);
  Serial.printf("pubmsg %s\n",lastwill);
